...
drainage_density_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_WRD/Drain_WRD.tif")
aquifer_recharge_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_WRD/Geo_WRD.tif")
rainfall_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_WRD/Rain_WRD.tif")
land_use_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_WRD/Land_WRD.tif")
soil_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_WRD/Soil_WRD.tif")
slope_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_WRD/Slope_WRD.tif")
roads_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_WRD/Roads_WRD.tif")
wadis_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_WRD/Wadis_WRD.tif")
population_density_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_WRD/Pop_WRD.tif")
...
png("sensitivity_equal_100_WRD.png", width = 1200, height = 1000, res = 150)  # Set width, height, and resolution
par(mar = c(5, 5, 4, 2))  # Adjust the margins
plot(sensitivity_raster_expert, 
     main = "Sensitivity Map for WRD (Expert Weights)", 
...
plot(sensitivity_raster_equal, 
     main = "Sensitivity Map for WRD (Equal Weights)", 
     cex.main = 2)  # Larger title size for better visibility
dev.off()  # Close the PNG device

# Optional: Save the rasters to files if needed
writeRaster(sensitivity_raster_expert, "sensitivity_expert_WRD_100.tif", overwrite = TRUE)
writeRaster(sensitivity_raster_equal, "sensitivity_equal_WRD_100.tif", overwrite = TRUE)
...
ggsave("variance_comparison_WRD_100.png", plot = plot_variance, width = 10, height = 6, dpi = 300)
...
# Set the order of the Layer factor to ensure Expert and Equal are on the left
combined_sensitivity_stats$Layer <- factor(
  combined_sensitivity_stats$Layer, 
  levels = c("Expert Weighted", "Equal Weighted", layer_names)  # Specify the order explicitly
)

# Plot with corrected grouping and color scheme
ggplot(combined_sensitivity_stats, aes(x = Layer, y = Mean, fill = Type)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  geom_errorbar(aes(ymin = Mean - Adjusted_SD, ymax = Mean + Adjusted_SD), 
                width = 0.2, color = "black") +
