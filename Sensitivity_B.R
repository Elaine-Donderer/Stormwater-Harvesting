...
drainage_density_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_B/Drain_B.tif")
aquifer_recharge_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_B/Geo_B.tif")
rainfall_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_B/Rain_B.tif")
land_use_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_B/Land_B.tif")
soil_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_B/Soil_B.tif")
slope_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_B/Slope_B.tif")
roads_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_B/Roads_B.tif")
wadis_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_B/Wadis_B.tif")
population_density_layer <- raster("C:/Users/Elain/Downloads/Sensitivity_B/Pop_B.tif")
...
# Inspect the first few values of the raster
head(values(drainage_density_layer))
plot(drainage_density_layer)
plot(aquifer_recharge_layer)
...
# Empty list to store masked layers
masked_layers <- list()

# Iterate over each layer, convert to SpatRaster, mask, and store the results
for (i in seq_along(layers)) {
  layer_terra <- rast(layers[[i]])  # Convert the layer to SpatRaster (terra)
  masked_layer <- mask(layer_terra, study_area_terra)  # Apply the mask
  masked_layers[[i]] <- masked_layer  # Store the masked layer in the list
  
  # Plot the masked layer
...
for (j in 1:length(layers)) {
  print(paste("Original Layer", j))
  print(summary(layers[[j]]))  # Check the summary stats of the original layers
  print(ext(layers[[j]]))  # Check the extent of the original layers
  print(res(layers[[j]]))  # Check the resolution of the original layers
}
...
png("sensitivity_expert_100_B.png", width = 1200, height = 1000, res = 150)  # Set width, height, and resolution
par(mar = c(5, 5, 4, 2))  # Adjust the margins
plot(sensitivity_raster_expert, 
     main = "Sensitivity Map for B (Expert Weights)", 
     cex.main = 2)  # Larger title size for better visibility
dev.off()  # Close the PNG device

# Save the plot as a larger image
png("sensitivity_equal_100_B.png", width = 1200, height = 1000, res = 150)  # Set width, height, and resolution
par(mar = c(5, 5, 4, 2))  # Adjust the margins
plot(sensitivity_raster_equal, 
     main = "Sensitivity Map for B (Equal Weights)", 
     cex.main = 2)  # Larger title size for better visibility
dev.off()  # Close the PNG device

# Optional: Save the rasters to files if needed
writeRaster(sensitivity_raster_expert, "sensitivity_expert_B_100.tif", overwrite = TRUE)
writeRaster(sensitivity_raster_equal, "sensitivity_equal_B_100.tif", overwrite = TRUE)
...
# Number of layers (assuming 9 layers)
num_layers <- 9

# Initialize a list to store sensitivity results for each layer
sensitivity_by_layer <- list()
# Convert each layer in the raster stack to a matrix
suitability_matrices <- lapply(1:nlyr(suitability_stack), function(i) as.matrix(suitability_stack[[i]]))

# Check if conversion is successful
if (all(sapply(suitability_matrices, is.matrix))) {
  cat("Conversion of raster stack to matrices is successful.\n")
} else {
  stop("Failed to convert one or more layers to matrices.")
}
...
for (j in 1:num_layers) {
  ...
  suitability_results_layer <- matrix(0, nrow = nrow(suitability_matrices[[1]]), ncol = iterations)
  ...
  for (i in 1:iterations) {
    ...
    # Calculate suitability for this iteration with only layer 'j' varied
    suitability_results_layer[, i] <- rowSums(mapply(`*`, suitability_matrices, weights))
  }
...
cat("Sensitivity analysis by layer completed.\n")
...
ggsave("variance_comparison_B_100.png", plot = plot_variance, width = 10, height = 6, dpi = 300)

# Plot each layer to visually inspect its contents
par(mfrow = c(3, 3))  # Arrange the plots in a 3x3 grid for better comparison

for (i in 1:9) {
  plot(masked_layers[[i]], main = paste("Layer", i))
}

#troubleshoot:
# Printing the order of the layers as currently used in the variance calculation
print("Current Order of Layers (by index) and their calculated variances:")
print(layer_influence_summary)

# Check layer names to ensure they align with the raster layers
print("Layer names as initially assigned:")
print(layer_names)
print(layer_influence_summary)
